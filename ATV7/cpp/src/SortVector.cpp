...
void SortVector::__merge(int begin, int mid, int end){
	int n = end - begin + 1;
	int n1 = mid - begin + 1;
	int n2 = end - mid;
	string** aux = new string*[n];
    for(int i = 0; i < n; i++)
        aux[i] = array.data[i + begin];

	int i = 0, j = mid - begin + 1, k = begin;
	...
	while(i < n1) array.data[k++] = aux[i++];
	while(j < n) array.data[k++] = aux[j++];

	delete[] aux;
}

void SortVector::__hibrid_mergesort(int begin, int end){
	if (end - begin + 1 <= 24) {
		__insertion(begin, end);
	}else{
		int mid = (begin + end) >> 1;
		__hibrid_mergesort(begin, mid);
		__hibrid_mergesort(mid + 1, end);
